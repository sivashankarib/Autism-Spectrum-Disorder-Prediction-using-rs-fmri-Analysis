...
def res_unet(input_size, num_filters, kernel_size, num_channels, num_classes):
    ...
    # Train_X = np.zeros((Images.shape[0], IMG_SIZE, IMG_SIZE))

    # Test_X = np.zeros((GT.shape[0], IMG_SIZE, IMG_SIZE))
...
def Model_ResUnet(Images, GT, sol=None):
    if sol is None:
        sol = [4, 50, 0, 5, 0]
    IMG_SIZE = 224
    num_filters = 64
    ...
    activation = ['linear','Relu', 'tanh', 'sigmoid', 'Leaky Relu']
    Train_X = np.zeros((Images.shape[0], IMG_SIZE, IMG_SIZE, 3))

    Test_X = np.zeros((GT.shape[0], IMG_SIZE, IMG_SIZE, 3))

    encoder_output = encoder(Train_X, num_filters, kernel_size)
    model = res_block(encoder_output[-1], [num_filters[-1] * 2], kernel_size,
                       strides=[2, 1], name='bridge')
    model.compile(optimizer=optimizer[int(sol[2])], loss=keras.losses.binary_crossentropy, metrics=['accuracy'])
    model.summary()
    model.fit(Train_X, Test_X,
              epochs=sol[1], steps_per_epoch=2, verbose="auto")
