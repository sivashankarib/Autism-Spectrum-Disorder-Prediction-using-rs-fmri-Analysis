...
def MCO(Positions, fobj, VRmin, VRmax, Max_iter):
    ...
    while FEs <= Max_iter:
        ...
        for k in range(dim):
            ...
            Z[i.astype(int)] = VRmin[i.astype(int)] + np.random.rand(dim) * (VRmax[i.astype(int)] - VRmin[i.astype(int)])
            xx1 = int(np.var(Z[(i.astype(int))] < ub))
            # Z[xx1] = lb[xx1] + np.random.rand((xx1)) * (ub[xx1] - lb[xx1])
            Z[i.astype(int)] = VRmax[i.astype(int)] + np.random.rand(dim) * (VRmin[i.astype(int)] - VRmax[i.astype(int)])
